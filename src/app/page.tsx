...
import { Zap, Users, Trophy, ChevronRight, MapPin, Swords, Database } from "lucide-react"
...
export default function Home() {
  ...
  const [cachedTurfs, setCachedTurfs] = useState<any[]>([])
  
  // Real-time Feeds
  const turfsQuery = useMemoFirebase(() => query(collection(db, "turfs"), limit(8)), [db])
  ...
  const { data: turfs, loading: loadingTurfs } = useCollection(turfsQuery)
  const { data: challenges } = useCollection(challengesQuery)
  const { data: teams } = useCollection(teamsQuery)

  // Cache & Diagnostic Synchronization
  useEffect(() => {
    console.log(`[NETWORK PROTOCOL] Active Project: ${firebaseConfig.projectId}`);
    
    if (turfs && turfs.length > 0) {
      console.log(`[NETWORK SYNC] Home turf count: ${turfs.length}`);
      setCachedTurfs(turfs);
      localStorage.setItem('home_turf_cache', JSON.stringify(turfs));
    } else {
      const saved = localStorage.getItem('home_turf_cache');
      if (saved) setCachedTurfs(JSON.parse(saved));
    }
  }, [turfs]);

  const displayTurfs = (turfs && turfs.length > 0) ? turfs : cachedTurfs;
      ...
      <main className="flex-1 pb-32">
        ...
        <section className="px-4 max-w-7xl mx-auto mb-20">
          <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
            <h2 className="flex items-center gap-3 font-black uppercase italic"><Swords className="h-5 w-5 text-primary" /> Open Challenges</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {challenges?.map((challenge: any) => (
              <div key={challenge.id} className="bg-card border border-border p-6 flex flex-col justify-between hover:border-primary/40 transition-colors">
                <div>
        ...
        <section id="turfs" className="px-4 max-w-7xl mx-auto mb-20 scroll-mt-20">
          <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
            <h2 className="flex items-center gap-3 font-black uppercase italic"><Database className="h-5 w-5 text-primary" /> Top Venues</h2>
          ...
          {displayTurfs.length === 0 && !loadingTurfs && (
             <div className="py-20 text-center border border-dashed border-border rounded-xl">
                <Zap className="h-10 w-10 text-muted mx-auto mb-4 opacity-20" />
        ...
        <section className="px-4 max-w-7xl mx-auto mb-20">
          <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-4">
            <h2 className="flex items-center gap-3 font-black uppercase italic"><Users className="h-5 w-5 text-primary" /> Active Squads</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {teams?.map((team: any) => (
              <Link key={team.id} href={`/teams/${team.id}`} className="bg-card border border-border p-6 hover:border-primary group transition-all">
                <div className="h-12 w-12 bg-primary/10 flex items-center justify-center mb-4 text-primary font-black italic rounded-lg border border-primary/10">
