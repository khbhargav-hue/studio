        ...
        <div className="mx-auto max-w-7xl px-4">
            ...
            <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
              <DialogTrigger asChild>
                <Button className="h-16 px-10 bg-primary text-black font-black uppercase tracking-widest text-xs rounded-2xl shadow-2xl hover:scale-[1.02] transition-transform">
                  <Plus className="mr-2 h-5 w-5" /> CREATE YOUR SQUAD
                </Button>
              </DialogTrigger>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {newTeam.players.map((p, i) => (
                          <div key={i} className="px-4 py-2 bg-primary/10 border border-primary/20 text-primary rounded-xl text-xs font-bold uppercase flex items-center gap-2">
                            {p}
          ...
          {loading ? (
            ...
            <div className="text-center py-40 glass-card rounded-[5rem] border-dashed border-white/10 max-w-3xl mx-auto flex flex-col items-center gap-8">
              ...
              <div>
                <h3 className="text-3xl font-black text-white/10 uppercase italic">No Active Squads</h3>
                <p className="text-white/20 mt-4 max-w-xs mx-auto">Be the first to form a team and dominate the Mysuru circuits.</p>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    ...
    >
      <div className="p-8 pb-4 relative">
        <div className="absolute top-8 right-8 h-12 w-12 bg-white/5 rounded-2xl flex items-center justify-center border border-white/5">
          <ShieldCheck className="h-5 w-5 text-primary opacity-40" />
        </div>
        
        <div className="h-20 w-20 bg-primary/10 rounded-3xl flex items-center justify-center mb-8 border border-primary/20 shadow-[0_0_20px_rgba(57,255,20,0.1)]">
          {team.logoUrl ? (
            <img src={team.logoUrl} className="h-full w-full object-cover rounded-3xl" />
          ) : (
        ...
        <Link href={`/teams/${team.id}`}>
          <h3 className="text-3xl font-black italic uppercase tracking-tighter leading-tight group-hover:text-primary transition-colors">{team.teamName}</h3>
        </Link>
        <div className="flex items-center gap-2 mt-4 text-[10px] font-black uppercase tracking-widest text-white/40">
      ...
      <div className="p-8 pt-4 flex-1 space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
            <p className="text-[9px] font-black uppercase text-white/30 tracking-widest mb-1">Sport</p>
            <p className="text-xs font-bold text-white uppercase">{team.sport}</p>
          </div>
          <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
            <p className="text-[9px] font-black uppercase text-white/30 tracking-widest mb-1">Roster</p>
            <p className="text-xs font-bold text-white uppercase">{team.players?.length || 0} Players</p>
          </div>
        </div>

        <div className="bg-primary/5 p-4 rounded-2xl border border-primary/20">
          <p className="text-[9px] font-black uppercase text-primary/60 tracking-widest mb-1">Turf Preference</p>
          <p className="text-xs font-bold text-primary uppercase italic truncate">{team.turfPreference || "Flexible"}</p>
        </div>
      </div>
      ...
      <div className="p-8 pt-0 grid grid-cols-2 gap-3">
        ...
        <Button asChild className="h-14 rounded-2xl bg-[#25D366] hover:bg-[#20ba5a] text-white font-black uppercase tracking-widest text-[9px] border-none shadow-xl shadow-[#25D366]/20">
          <a href={whatsappUrl} target="_blank" rel="noopener noreferrer">
            <MessageCircle className="mr-2 h-4 w-4" /> CHALLENGE
          </a>
        </Button>
      </div>
    </motion.div>
