...
export function TurfCard({ turf }: TurfCardProps) {
  const whatsappUrl = `https://wa.me/${turf.whatsapp}?text=${encodeURIComponent(`Hi! I found ${turf.name} on Turfista and want to book a slot.`)}`;
  const displayImage = turf.imageUrl || "https://images.unsplash.com/photo-1529900748604-07564a03e7a6?w=800&q=75";

  return (
    <div className="group bg-card border border-border flex flex-col h-full hover:border-primary/40">
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-black">
        ...
        <div className="absolute bottom-2 right-2 bg-black/80 px-2 py-0.5 text-[10px] font-black text-white">
          ★ {turf.rating || 4.5}
        </div>
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col">
        <Link href={`/turf/${turf.id}`}>
          <h3 className="text-base font-black uppercase italic tracking-tighter hover:text-primary mb-1 truncate">
            {turf.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto space-y-3 pt-4 border-t border-border">
          <p className="text-[10px] font-black uppercase text-[#444] tracking-widest italic">
            Price varies — ask before booking
          </p>
          <Button 
            asChild 
            className="w-full h-12 bg-primary text-black font-black uppercase tracking-widest text-[10px] rounded-none shadow-2xl shadow-primary/10"
          >
            <a href={whatsappUrl} target="_blank" rel="noopener noreferrer">
              <MessageCircle className="h-4 w-4 mr-2" /> Ask Price & Book
            </a>
          </Button>
        </div>
      </div>
