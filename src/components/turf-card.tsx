...
export function TurfCard({ turf }: TurfCardProps) {
  ...
  const whatsappUrl = `https://wa.me/${turf.whatsappNumber}?text=${encodeURIComponent(`Hi! I want to book ${turf.name}`)}`;

  return (
    <div className={cn(
      "flat-card flex flex-col group",
      turf.isPopular && "border-[#FFD700] ring-1 ring-[#FFD700]/20"
    )}>
      <div className="relative aspect-[16/10] w-full rounded-xl overflow-hidden mb-6">
        <Image
          src={displayImage}
          alt={turf.name}
          fill
        ...
        />
        {turf.isPopular && (
          <div className="absolute top-3 left-3 bg-[#FFD700] text-black label-caps px-3 py-1 rounded-[6px] font-bold">
            Featured
          </div>
        )}
        <div className="absolute bottom-3 right-3 bg-background/80 backdrop-blur-md px-3 py-1 rounded-[6px] flex items-center gap-1.5 border border-border">
      ...
      <CardContent className="p-0 flex-1 flex flex-col space-y-4">
        <div>
          ...
          <Link href={`/turf/${turf.id}`}>
            <h3 className="text-[18px] font-semibold hover:text-primary mb-1 line-clamp-1">
              {turf.name}
            </h3>
          </Link>
        ...
        <div className="pt-4 border-t border-border flex items-center justify-between mt-auto">
          <div className="flex items-baseline gap-1">
            <span className="text-[20px] font-bold text-primary">₹{turf.pricePerHour}</span>
        ...
        <Button 
          ...
          className="whatsapp-btn w-full h-[48px] mt-4"
        >
